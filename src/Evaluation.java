public class Evaluation {
    ...
    static final int RookMobilityValue = 4; //random values
    static final int KnightMobilityValue = 3;
    static final int BishopMobilityValue = 6;
    static final int QueenMobilityValue = 4;
    static final int MGKingMobilityValue = -8; //punish king if able to move a lot

    static final int PawnProtectionPenalty = 25;
    static final int RookProtectionPenalty = 50;
    static final int KnightProtectionPenalty = 75;
    static final int BishopProtectionPenalty = 40;
    static final int QueenProtectionPenalty = 40;

    static final int[] kingPawnShield = {-50, 0, 40, 60, 70, 80, 90, 100};
    ...
    public static double evaluation(Board board) {
        int materialDiff;
        double positionalScore = 0; //gives bonuses based on how well your pieces are positioned using position tables
        double mobilityScore = 0;  //gives bonuses based on how many squares pieces attack
        double defenseScore = 0;
        double protectionPenalty = 0;
        double corneredKingFactor;
        ...
        while (currentBitBoard != 0) {
            ...
            selectedPieceBit = 1L<<selectedPiece;
            currentBitBoard &= ~selectedPieceBit;

            fPCount++;
            positionalScore += (mgPawnTable[fIndex][selectedPiece] * (256 - endGameFactor) + egPawnTable[fIndex][selectedPiece] * endGameFactor) / 256;

            if ((selectedPieceBit & board.fAttackMask) == 0) {
                protectionPenalty -= PawnProtectionPenalty;
            }
        ...
        while (currentBitBoard != 0) {
            ...
            if ((selectedPieceBit & board.eAttackMask) == 0) {
                protectionPenalty += PawnProtectionPenalty;
            }
        ...
        while (currentBitBoard != 0) {
            ...
            if ((selectedPieceBit & board.fAttackMask) == 0) {
                protectionPenalty -= RookProtectionPenalty;
            }
        ...
        while (currentBitBoard != 0) {
            ...
            if ((selectedPieceBit & board.fAttackMask) == 0) {
                protectionPenalty -= KnightProtectionPenalty;
            }
        }
        ...
        while (currentBitBoard != 0) {
            ...
            if ((selectedPieceBit & board.fAttackMask) == 0) {
                protectionPenalty -= BishopProtectionPenalty;
            }
        }
        ...
        while (currentBitBoard != 0) {
            ...
            if ((selectedPieceBit & board.fAttackMask) == 0) {
                protectionPenalty -= QueenProtectionPenalty;
            }
        }
        ...
        positionalScore /= 2;
        protectionPenalty = protectionPenalty*(256-endGameFactor) /256;
        defenseScore += (fAttackersOnKing * fAttackersOnKing * fAttackersOnKing * 5) - (eAttackersOnKing * eAttackersOnKing * eAttackersOnKing * 5);

        int kingMobility = BitMethods.countBits(MoveGeneration.singleQueenAttacks(fKSquare, board.occupied) & inverseFOccupied);
        positionalScore += (mgKingTable[fIndex][fKSquare] * (256 - endGameFactor) + egKingTable[fIndex][fKSquare] * endGameFactor) / 256;
        mobilityScore += kingMobility * MGKingMobilityValue * (256 - endGameFactor) / 256;
        defenseScore += kingPawnShield[BitMethods.countBits(fKingBubble & board.fPawn)] * (256 - endGameFactor) / 256;

        kingMobility = BitMethods.countBits(MoveGeneration.singleQueenAttacks(eKSquare, board.occupied) & inverseEOccupied);
        positionalScore -= (mgKingTable[eIndex][eKSquare] * (256 - endGameFactor) + egKingTable[eIndex][eKSquare] * endGameFactor) / 256;
        mobilityScore -= kingMobility * MGKingMobilityValue * (256 - endGameFactor) / 256;
        ...
        corneredKingFactor = endKingCornerEval(fKSquare, eKSquare) * endGameFactor * 2 / 256;
        if ((eRCount+eQCount) > (fRCount+fQCount)) {
            corneredKingFactor *= -1;
        }

        //System.out.printf("materialDiff: %d\npositionalScore: %.1f\ncorneredKingFactor: %.1f8\nmobilityScore: %.1f\ndefenseScore: %.1f\nprotectionPenalty: %.1f\nEval: %.1f\n", materialDiff, positionalScore, corneredKingFactor, mobilityScore, defenseScore, protectionPenalty, materialDiff + positionalScore + corneredKingFactor + mobilityScore + defenseScore + protectionPenalty);
        return materialDiff + positionalScore + corneredKingFactor + mobilityScore + defenseScore + protectionPenalty;

    }

    public static int endKingCornerEval(int fKing, int eKing) {
