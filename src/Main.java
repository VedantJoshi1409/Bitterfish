...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //Repetition test: rn1qkb1r/pp2pppp/2pp1n2/8/3PP1b1/2N2N1P/PPP2PP1/R1BQKB1R b KQkq -
        ...
        frame = new GameFrame();
        //Client.returnMove();
        computerGame(false, 500);
        //engineTest(100);
    }

    public static void initialize() {
        MagicBitboards.initializeArrays();
        System.out.println("Magic Bitboards Initialized");
        Zobrist.initializeRandomKeys();
        System.out.println("Zobrist Keys Initialized");
        OpeningFinder.initializeOpenings();
        System.out.println("Opening Book Initialized");
    }
    ...
    public static void engineTest(int thinkTime) {
        int theoryLimit = (int) (Math.random()*10);
        if (theoryLimit == 0) {
            inTheory = false;
        }
        int counter = 0;
    ...
    public static boolean checkEndGame() {
        int whitePieces = Evaluation.countBits(wRook) + Evaluation.countBits(wKnight) + Evaluation.countBits(wBishop) + (Evaluation.countBits(wQueen)*2);
        int blackPieces = Evaluation.countBits(bRook) + Evaluation.countBits(bKnight) + Evaluation.countBits(bBishop) + (Evaluation.countBits(bQueen)*2);
        return whitePieces<3 && blackPieces<3;
    }

    public static void printBoard(Long[] board) {
        for (long currentLong: board) {
